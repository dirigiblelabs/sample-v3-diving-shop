...
.service('Perspectives', ['$resource', function($resource){
	return $resource('/services/v3/js/diving-shop/api/launchpad/perspectives.js');
}])
.service('Tiles', ['$resource', function($resource){
	return $resource('/services/v3/js/diving-shop/api/launchpad/tiles.js');
}])
.service('Menu', ['$resource', function($resource){
	return $resource('/services/v3/js/diving-shop/api/launchpad/menu.js');
}])
...
.factory('viewRegistry', ['ViewRegistrySvc', '$resource', 'ViewFactories', function(ViewRegistrySvc, $resource, ViewFactories){
	...
	var get = function(viewsExtensionPoint){
		return $resource('/services/v3/js/diving-shop/api/launchpad/views.js').query({'extensionPoint': viewsExtensionPoint}).$promise
				.then(function(data){
...
.directive('menu', ['$resource', 'Theme', 'User', 'Layouts', 'Menu', 'messageHub', function($resource, Theme, User, Layouts, Menu, messageHub){
	return {
		...
		templateUrl: '/services/v3/web/diving-shop/ui/resources/templates/menu.html'
	}
}])
.directive('sidebar', ['Perspectives', function(Perspectives){
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.activePerspective = localStorage.getItem('DIRIGIBLE.application.diving-shop.activePerspective');

			scope.setActivePerspective = function(activePerspective) {
				localStorage.setItem('DIRIGIBLE.application.diving-shop.activePerspective', activePerspective);
				scope.activePerspective = activePerspective;
			};
		},
		templateUrl: '/services/v3/web/diving-shop/ui/resources/templates/sidebar.html'
	}
}])
.directive('tiles', ['Tiles', function(Tiles){
	return {
		...
		templateUrl: '/services/v3/web/diving-shop/ui/resources/templates/tiles.html'
	}
}])
